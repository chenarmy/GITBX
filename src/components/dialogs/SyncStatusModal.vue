...
const { t } = useI18n();
async function locate(commitId: string) { await repoStore.locateCommit(commitId); repoStore.closeModal('syncStatus'); }
</script>

<template>
  <div
    v-if="repoStore.isSyncStatusOpen"
    class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
    @keydown.esc.window="repoStore.closeModal('syncStatus')"
  >
    <div
      role="dialog"
      aria-modal="true"
      :aria-label="t('Incoming and Outgoing Commits')"
      class="w-full max-w-4xl h-[65vh] bg-card border border-border rounded-xl shadow-2xl overflow-hidden flex flex-col text-xs"
      @click.stop
    >
      <div class="h-11 px-4 flex items-center justify-between border-b border-border bg-muted/50">
        ...
        <button
          ...
          @click="repoStore.closeModal('syncStatus')"
        >
      ...
      <div class="flex-1 min-h-0 grid grid-cols-2 divide-x divide-border">
        <section class="flex flex-col min-h-0">
          <header class="px-3 py-2 font-bold text-emerald-500 flex gap-2"><ArrowDownCircle class="w-4 h-4" aria-hidden="true" />{{ t('Incoming') }} ({{ repoStore.syncStatus.incoming.length }})</header>
        ...
        <section class="flex flex-col min-h-0">
          <header class="px-3 py-2 font-bold text-emerald-500 flex gap-2"><ArrowUpCircle class="w-4 h-4" aria-hidden="true" />{{ t('Outgoing') }} ({{ repoStore.syncStatus.outgoing.length }})</header>
