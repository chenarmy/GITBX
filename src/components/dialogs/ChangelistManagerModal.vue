...
<template>
  <div
    v-if="store.isManagerOpen"
    class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
    @keydown.esc.window="store.isManagerOpen = false"
  >
    <div
      role="dialog"
      aria-modal="true"
      :aria-label="t('Changelists')"
    ...
    >
      <div class="h-11 px-4 flex items-center justify-between border-b border-border bg-muted/50">
        <div class="flex gap-2 items-center">
          <ListTodo class="w-4 h-4 text-violet-500" aria-hidden="true" />
          <span class="font-bold text-sm">{{ t('Changelists') }}</span>
        </div>
        <button
          type="button"
          aria-label="Close dialog"
          class="p-1 rounded hover:bg-accent focus:outline-none focus:ring-1 focus:ring-foreground/50"
          @click="store.isManagerOpen = false"
        >
